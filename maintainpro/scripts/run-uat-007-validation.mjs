...
function cleanWebBuildCache() {
  const nextDir = path.join(root, "apps", "web", ".next");
  if (existsSync(nextDir)) {
    spawnSync(process.execPath, ["-e", `require('fs').rmSync(${JSON.stringify(nextDir)},{recursive:true,force:true})`], {
      cwd: root,
      stdio: "ignore"
    });
  }
...
run("work_order_validation_tests", npmCmd, [
  "run",
  "test",
  "--workspace",
  "@maintainpro/api",
  "--",
  "--runInBand",
  "test/work-order-validation.spec.ts",
  "test/work-order-assignees-leave.spec.ts"
], {}, npmShell);
run("smoke_deploy", process.execPath, ["scripts/run-smoke-with-staging-env.mjs"]);
console.log("uat_007_validation=complete");
