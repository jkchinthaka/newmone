...
const counts = { VERIFIED: 0, PARTIAL: 0, MISSING: 0, EXTERNAL_DEPENDENCY: 0 };
const lines = [];
lines.push("# MaintainPro Enterprise Requirements Traceability Matrix");
lines.push("");
lines.push("Branch: maintainpro/phase-15-sqlserver-migration  ");
lines.push("Updated: 2026-09-17 (enterprise governance completion)  ");
lines.push("");
lines.push("Status legend: **VERIFIED** | **PARTIAL** | **MISSING** | **EXTERNAL_DEPENDENCY**");
lines.push("");
lines.push("| ID | Requirement Area | Existing Implementation | Status | Evidence | Missing Work | Tests |");
lines.push("| -- | ---------------- | ----------------------- | ------ | -------- | ------------ | ----- |");

for (const r of areas) {
  counts[r[3]] = (counts[r[3]] || 0) + 1;
  lines.push(`| ${r[0]} | ${r[1]} | ${r[2]} | ${r[3]} | ${r[4]} | ${r[5] || ""} | ${r[6] || ""} |`);
}
...
lines.push(`- **EXTERNAL_DEPENDENCY**: ${counts.EXTERNAL_DEPENDENCY}`);
lines.push("");
lines.push("## External dependencies (narrow)");
lines.push("");
lines.push("- Production Bileeta ERP API credentials/contracts (adapters/mocks/exception center **implemented**)");
lines.push("- Production Microsoft Entra tenant registration (SSO adapter **implemented** in-code)");
lines.push("- Production SMTP/SMS provider credentials (notification dispatchers **implemented**)");
lines.push("");

fs.writeFileSync("docs/ENTERPRISE_REQUIREMENTS_MATRIX.md", lines.join("\n"), "utf8");
console.log(counts);
