...
| Work template snapshot on WO | PARTIAL | ChecklistExecution + PM revision; JobCode not snapped |
...
- _(pending)_ feat(admin): config history, reason/fault codes, SLA-driven WO deadlines
...
- focused tests: `maintenance-config-sla`, `job-domain.util`, `auth-cookie-size` — pass
