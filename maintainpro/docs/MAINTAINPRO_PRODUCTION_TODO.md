...
| DEPLOY-003B | P1 | Platform | Align Render staging DB + hosted smoke retry | DONE | render.yaml DB names + local re-seed + smoke retry | render.yaml, seed.ts | Blueprint names fixed; Render dashboard `DATABASE_URL` updated; hosted login now succeeds (~18.8s). |
...
| UAT-001 | P1 | QA | Manual browser UAT checklist | IN_PROGRESS | FINAL_UAT_AND_CUTOVER_CHECKLIST Part 2 | staging web | 2026-06-15 re-test: wrong-password UX PASS on staging; login/smoke FAIL (credential mismatch); `/admin`+#310 fix PASS local e2e; full sign-off blocked until hosted seed/smoke password aligned. |
