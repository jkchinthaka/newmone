...
|---|---|---|---|---|---|---|---|
| SEC-001 | P0 | Auth UI | Remove public demo credentials from login page | NOT_STARTED | apps/web/app/(auth)/login/page.tsx | manual UI check | Audit found NO hardcoded creds shown today — task is mostly about confirming/staging-gating, low effort |
...
| SEC-007 | P0 | Tenancy | Fix UtilitiesService tenant leak (meters, meter, allReadings) | NOT_STARTED | apps/api/src/modules/utilities/utilities.service.ts:10,14,65 | tenant-isolation jest test | Confirmed: all 3 methods missing tenantId filter |
| SEC-008 | P0 | Backend | Protect Swagger (/api/docs) by NODE_ENV/auth; minimal public /health | NOT_STARTED | apps/api/src/main.ts:121-135 | manual: hit /api/docs in prod build | Confirmed open with no gating; /health & /health/readiness already exist |
| SEC-009 | P0 | Auth | Reject inactive users in jwt.strategy.ts validate() | NOT_STARTED | apps/api/src/modules/auth/jwt.strategy.ts:33-35 | jest: deactivated user token rejected | Confirmed isActive not checked on token validate |
