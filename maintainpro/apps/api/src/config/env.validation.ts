...
export const envValidationSchema = Joi.object({
  ...
  BACKUP_RESTORE_REQUIRED_FOR_READINESS: Joi.boolean().default(false),
  RECOVERY_BACKUP_POLICY_CONFIGURED: Joi.alternatives().try(Joi.boolean(), Joi.string()).optional(),
  RECOVERY_LAST_BACKUP_INTEGRITY_STATUS: Joi.string().max(64).optional().allow(""),
  RECOVERY_LAST_BACKUP_AGE_HOURS: Joi.alternatives().try(Joi.number(), Joi.string()).optional().allow(""),
  RECOVERY_LAST_RESTORE_TEST_STATUS: Joi.string().max(64).optional().allow(""),
  RECOVERY_LAST_RESTORE_TEST_AGE_HOURS: Joi.alternatives().try(Joi.number(), Joi.string()).optional().allow(""),
  RECOVERY_EVIDENCE_SOURCE: Joi.string().valid("e2e", "production", "staging", "").optional().allow(""),
  ...
  QUEUE_STARTUP_RECONCILIATION_ENABLED: Joi.boolean().default(true),
  QUEUE_RECONCILE_BATCH_SIZE: Joi.number().integer().min(1).max(200).default(50),
  OPERATIONAL_ALERT_EVALUATOR_ENABLED: Joi.boolean().default(true),
  OPERATIONAL_ALERT_EVALUATOR_INTERVAL_MS: Joi.number().integer().min(15000).default(60000),
  OPERATIONAL_ALERT_COOLDOWN_SECONDS: Joi.number().integer().min(30).default(300),
  OPERATIONAL_ALERT_NOTIFICATION_MODE: Joi.string().valid("disabled", "mock", "uat").default("disabled"),
  ...
  BILEETA_API_USERNAME: Joi.string().allow(""),
  BILEETA_API_PASSWORD: Joi.string().allow(""),
  BILEETA_API_KEY: Joi.string().allow(""),
  ...
  /** Dedicated HS256 secret for MaintainPro → FG SSO assertions (min 32 chars when FG SSO used). */
  FG_SSO_SIGNING_SECRET: Joi.string().allow("").default(""),
  FG_SSO_SHARED_SECRET: Joi.string().allow("").default(""),
