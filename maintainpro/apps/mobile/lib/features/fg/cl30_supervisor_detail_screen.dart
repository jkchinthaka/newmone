...
class Cl30SupervisorDetailScreen extends ConsumerStatefulWidget {
  const Cl30SupervisorDetailScreen({super.key, required this.submissionId});

  final String submissionId;

  @override
  ConsumerState<Cl30SupervisorDetailScreen> createState() =>
      _Cl30SupervisorDetailScreenState();
}

class _Cl30SupervisorDetailScreenState
    extends ConsumerState<Cl30SupervisorDetailScreen> {
  final _noteController = TextEditingController();
  final _guard = InFlightGuard();
  FgReviewDetail? _detail;
  bool _loading = true;
  bool _busy = false;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final detail =
          await ref.read(fgApiClientProvider).getReview(widget.submissionId);
      if (!mounted) return;
      setState(() {
        _detail = detail;
        _loading = false;
  ...
  Future<void> _decide(String decision) async {
    ...
    try {
      final outcome = await _guard.run(() async {
        // Server idempotencyKey for review decision.
        final key = const Uuid().v4();
        return ref.read(fgApiClientProvider).reviewDecision(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Supervisor review')),
      body: _loading
  ...
  Widget _buildBody() {
    ...
    return ListView(
      ...
      children: [
        ...
        if (d.selfReviewBlocked) ...[
          const SizedBox(height: MpSpacing.md),
          MpErrorState(
            title: 'Self-review blocked',
            message: d.selfReviewMessage ??
                'Self-review is prohibited by governance policy.',
          ),
        ],
        ...
        if (d.canDecide) ...[
          const MpSectionHeader(title: 'Decision'),
          MpTextField(
          ...
          MpButton(
            ...
            isLoading: _busy,
            onPressed: _busy ? null : () => _decide('RETURNED_FOR_CORRECTION'),
          ),
        ],
      ],
    );
