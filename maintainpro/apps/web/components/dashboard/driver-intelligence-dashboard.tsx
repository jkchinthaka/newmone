...
export function DriverIntelligenceDashboard() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Driver intelligence</h3>
          <p className="text-sm text-slate-500">
            Fleet cost pressure, fuel flags, and assignment readiness from live driver intelligence data.
          </p>
        </div>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Link
            href={"/reports/driver-intelligence" as Route}
            className="inline-flex items-center gap-2 rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-sm font-medium text-brand-700 shadow-sm transition hover:bg-brand-100"
          >
            Reports <ArrowRight size={15} aria-hidden="true" />
          </Link>
        </div>
      </div>
      ...
      {query.isLoading ? (
        <StatePanel
          type="loading"
          title="Loading driver intelligence"
          message="Calculating live driver, cost, and fuel intelligence."
        />
      ) : query.isError ? (
        <StatePanel
          type="error"
          title="Could not load driver intelligence"
          message={toSafeApiErrorMessage(query.error, "The dashboard API returned an error.")}
          ...
          <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                <ShieldAlert size={18} className="text-rose-500" aria-hidden="true" />
              </div>
              <div className="mt-3 h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
          ...
          <div className="grid gap-4 xl:grid-cols-3">
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center gap-2 text-slate-900">
                <Trophy size={17} className="text-amber-500" aria-hidden="true" />
                  ...
                  dashboard.bestDrivers.map((driver) => (
                    <div key={driver.driverId} className="rounded-lg border border-slate-200 px-3 py-3">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-600">
                        <p>
                          Ranking score: <span className="font-semibold text-slate-900">{driver.rankingScore}</span>
                        </p>
                        <p>
                          Driver score: <span className="font-semibold text-slate-900">{driver.driverScore}</span>
                        </p>
                      </div>
                    </div>
                  ))
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center gap-2 text-slate-900">
                <AlertTriangle size={17} className="text-rose-500" aria-hidden="true" />
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-rose-700">
                            Score {driver.driverScore} · {driver.riskLevel}
                          </p>
                        </div>
                      </div>
                      <div className="mt-2 space-y-1 text-xs text-rose-800">
                        {driver.primaryReasons.length === 0 ? (
                          <p>No primary reasons returned.</p>
                        ) : (
                          driver.primaryReasons.map((reason) => <p key={reason}>{reason}</p>)
                        )}
                      </div>
                    </div>
                  ))
                ...
                dashboard.driverProfiles.slice(0, 6).map((driver) => (
                  <div key={driver.id} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-semibold ${driver.riskLevel === "LOW" ? "bg-emerald-100 text-emerald-700" : driver.riskLevel === "MEDIUM" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"}`}
                      >
                        {driver.riskLevel}
                      </span>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-600">
                      ...
                      <p>
                        Vehicle care: <span className="font-semibold text-slate-900">{driver.components.vehicleCareScore}</span>
                      </p>
                      <p>
                        Fuel score: <span className="font-semibold text-slate-900">{driver.components.fuelEfficiencyScore}</span>
                      </p>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-500">
                      ...
                      <p>
                        Trips: {driver.summary.completedTrips}/{driver.summary.totalTrips}
                      </p>
        ...
        <StatePanel type="empty" title="No driver intelligence data" message="No analytics are available for the selected range." />
      )}
    </div>
  );
}
